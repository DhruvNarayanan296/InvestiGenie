    ...
    if data is not None:
        ...
        # Add a new tab for Educational Resources
            ...
            **Disclaimer**: The educational content provided here is for informational purposes only and should not be considered as financial advice. Always consult with a professional financial advisor before making any investment decisions.
            ...
            # Function to manage query params and assign UserID
            def get_user_id():
                query_params = st.experimental_get_query_params()
                if 'UserID' in query_params:
                    st.session_state['UserID'] = query_params['UserID'][0]
                else:
                    new_user_id = str(uuid.uuid4())
                    st.session_state['UserID'] = new_user_id
                    # Instead of using set_query_params, manually adjust the query params
                    st.experimental_set_query_params(UserID=new_user_id)
            ...
            if 'data_loaded' not in st.session_state:
                c.execute('SELECT amount FROM cash WHERE UserID = ?', (st.session_state['UserID'],))
                cash_data = c.fetchone()
                if cash_data:
                    st.session_state['portfolio'] = {'cash': cash_data[0], 'stocks': {}}
                else:
                    st.session_state['portfolio'] = {'cash': 1000000, 'stocks': {}}
                    c.execute('INSERT INTO cash (UserID, amount) VALUES (?, ?)', (st.session_state['UserID'], st.session_state['portfolio']['cash']))
                    conn.commit()

                for row in c.execute('SELECT ticker, shares, average_price FROM portfolio WHERE UserID = ?', (st.session_state['UserID'],)):
                    st.session_state['portfolio']['stocks'][row[0]] = {'shares': row[1], 'average_price': row[2]}

                st.session_state['transactions'] = []
                for row in c.execute('SELECT ticker, shares, price, type FROM transactions WHERE UserID = ?', (st.session_state['UserID'],)):
            ...
            def update_portfolio_db():
                c.execute('DELETE FROM portfolio WHERE UserID = ?', (st.session_state['UserID'],))
                c.execute('DELETE FROM cash WHERE UserID = ?', (st.session_state['UserID'],))
                c.execute('DELETE FROM transactions WHERE UserID = ?', (st.session_state['UserID'],))
                c.execute('INSERT INTO cash (UserID, amount) VALUES (?, ?)', (st.session_state['UserID'], st.session_state['portfolio']['cash']))
                for ticker, stock_data in st.session_state['portfolio']['stocks'].items():
                    c.execute('INSERT INTO portfolio (UserID, ticker, shares, average_price) VALUES (?, ?, ?, ?)', 
                            (st.session_state['UserID'], ticker, stock_data['shares'], stock_data['average_price']))
                for transaction in st.session_state['transactions']:
                    c.execute('INSERT INTO transactions (UserID, ticker, shares, price, type) VALUES (?, ?, ?, ?, ?)', 
                            (st.session_state['UserID'], transaction['ticker'], transaction['shares'], transaction['price'], transaction['type']))
                conn.commit()
            ...
            def main():
                ...
                if st.button("Sell"):
                    if ticker and sell_shares > 0:
                        if ticker in st.session_state['portfolio']['stocks'] and st.session_state['portfolio']['stocks'][ticker]['shares'] >= sell_shares:
                            ...
                            st.error("Insufficient shares to complete the transaction.")

                st.subheader("📊 Your Portfolio")
                st.markdown(f"**Cash:** 💵 ${st.session_state['portfolio']['cash']:,.2f}")
                portfolio_value = calculate_portfolio_value()
                portfolio_value_percentage_change = (((portfolio_value - 1000000) / 1000000) * 100)
                color = 'green' if portfolio_value_percentage_change >= 0 else 'red'
                st.markdown(f"**Portfolio Value:** ${portfolio_value:,.2f} <span style='color:{color}'>({portfolio_value_percentage_change:.2f}%)</span>", unsafe_allow_html=True)

                if st.session_state['portfolio']['stocks']:
                    st.markdown("### Stocks:")
                    for stock, stock_data in st.session_state['portfolio']['stocks'].items():
                        current_price = fetch_current_price(stock)
                        price_change_percentage = ((current_price - stock_data['average_price']) / stock_data['average_price']) * 100
                        color = 'green' if price_change_percentage >= 0 else 'red'
                        st.markdown(f"{stock}: {stock_data['shares']} shares at an average price of ${stock_data['average_price']:,.2f} <span style='color:{color}'>({price_change_percentage:.2f}%)</span>", unsafe_allow_html=True)
                ...
                if st.session_state['transactions']:
                    ...
                    for transaction in st.session_state['transactions']:
                        transaction_type = "Bought" if transaction['type'] == 'buy' else "Sold"
                        transaction_color = "green" if transaction['type'] == 'buy' else "red"
                        current_price = fetch_current_price(transaction['ticker'])
                        price_change_percentage = ((current_price - transaction['price']) / transaction['price']) * 100
                        color = 'green' if price_change_percentage >= 0 else 'red'
                        st.markdown(f"<span style='color:{transaction_color}'>{transaction_type}</span> {transaction['shares']} shares of {transaction['ticker']} at ${transaction['price']:,.2f} per share. Current price: ${current_price:,.2f} <span style='color:{color}'>({price_change_percentage:.2f}%)</span>", unsafe_allow_html=True)
                else:
                    st.write("No transactions found.")

                st.subheader("📄 Export Portfolio to PDF")
                if st.button("Export to PDF"):
                    pdf_data = export_to_pdf(portfolio_value)
                    with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
                        tmp_file.write(pdf_data)
                        tmp_file.seek(0)
                        b64 = base64.b64encode(tmp_file.read()).decode()
                        href = f'<a href="data:application/octet-stream;base64,{b64}" download="portfolio_report.pdf">Download PDF</a>'
