        ...
        # Add a new tab for Educational Resources
            ...
            **Disclaimer**: The educational content provided here is for informational purposes only and should not be considered as financial advice. Always consult with a professional financial advisor before making any investment decisions.
            ...
            def add_userid_column():
                c.execute('''ALTER TABLE portfolio ADD COLUMN UserID TEXT''')
                c.execute('''ALTER TABLE cash ADD COLUMN UserID TEXT''')
                c.execute('''ALTER TABLE transactions ADD COLUMN UserID TEXT''')
            ...
            c.execute('SELECT amount FROM cash WHERE UserID = ?', (UserID,))
            cash_data = c.fetchone()
            if cash_data:
                st.session_state['portfolio']['cash'] = cash_data[0]
            else:
                c.execute('INSERT INTO cash (amount, UserID) VALUES (?, ?)', (st.session_state['portfolio']['cash'], UserID))
                conn.commit()

            for row in c.execute('SELECT ticker, shares, average_price FROM portfolio WHERE UserID = ?', (UserID,)):
                st.session_state['portfolio']['stocks'][row[0]] = {'shares': row[1], 'average_price': row[2]}

            for row in c.execute('SELECT ticker, shares, price, type FROM transactions WHERE UserID = ?', (UserID,)):
            ...
            def update_portfolio_db():
                c.execute('DELETE FROM portfolio WHERE UserID = ?', (UserID,))
                c.execute('DELETE FROM cash WHERE UserID = ?', (UserID,))
                c.execute('DELETE FROM transactions WHERE UserID = ?', (UserID,))
                c.execute('INSERT INTO cash (amount, UserID) VALUES (?, ?)', (st.session_state['portfolio']['cash'], UserID))
                for ticker, stock_data in st.session_state['portfolio']['stocks'].items():
                    c.execute('INSERT INTO portfolio (ticker, shares, average_price, UserID) VALUES (?, ?, ?, ?)', 
                            (ticker, stock_data['shares'], stock_data['average_price'], UserID))
                for transaction in st.session_state['transactions']:
                    c.execute('INSERT INTO transactions (ticker, shares, price, type, UserID) VALUES (?, ?, ?, ?, ?)', 
                            (transaction['ticker'], transaction['shares'], transaction['price'], transaction['type'], UserID))
                conn.commit()
            ...
            def main():
                ...
                portfolio_percentage_change_color = 'green' if portfolio_value_percentage_change > 0 else 'red'
                st.markdown(f"**Total Portfolio Value:** 📈 ${portfolio_value:,.2f} | **Change:** <span style='color:{portfolio_percentage_change_color};'>{portfolio_value_percentage_change:.2f}%</span>", unsafe_allow_html=True)

                st.markdown("### 📋 Stocks Owned:")
                for stock, stock_data in st.session_state['portfolio']['stocks'].items():
                    current_price = fetch_current_price(stock)
                    if current_price:
                        # Calculate percentage change
                        percentage_change = ((current_price - stock_data['average_price']) / stock_data['average_price']) * 100
                        color = 'green' if percentage_change > 0 else 'red'
                        st.markdown(f"- **{stock}:** {stock_data['shares']} shares at an average price of ${stock_data['average_price']:,.2f}, Current Price: ${current_price:,.2f}  |  **Change:** <span style='color:{color};'>{percentage_change:.2f}%</span>", unsafe_allow_html=True)

                st.subheader("📜 Transaction History")
                for transaction in st.session_state['transactions']:
                    st.write(f"{transaction['type'].capitalize()} {transaction['shares']} shares of {transaction['ticker']} at ${transaction['price']:,.2f} per share")
