        ...
        # Add a new tab for Educational Resources
            ...
            **Disclaimer**: The educational content provided here is for informational purposes only and should not be considered as financial advice. Always consult with a professional financial advisor before making any investment decisions.
            ...
            c.execute('''CREATE TABLE IF NOT EXISTS portfolio
                        (ticker TEXT, shares REAL, average_price REAL)''')
            c.execute('''CREATE TABLE IF NOT EXISTS cash (amount REAL)''')
            c.execute('''CREATE TABLE IF NOT EXISTS transactions
                        (ticker TEXT, shares REAL, price REAL, type TEXT)''')

            # Load existing data from the database
            if 'data_loaded' not in st.session_state:
                c.execute('SELECT amount FROM cash')
                cash_data = c.fetchone()
                if cash_data:
                    st.session_state['portfolio']['cash'] = cash_data[0]
                else:
                    c.execute('INSERT INTO cash (amount) VALUES (?)', (st.session_state['portfolio']['cash'],))
                    conn.commit()
                    
                for row in c.execute('SELECT ticker, shares, average_price FROM portfolio'):
                    st.session_state['portfolio']['stocks'][row[0]] = {'shares': row[1], 'average_price': row[2]}
                    
                for row in c.execute('SELECT ticker, shares, price, type FROM transactions'):
            ...
            def update_portfolio_db():
                ...
                for ticker, stock_data in st.session_state['portfolio']['stocks'].items():
                    c.execute('INSERT INTO portfolio (ticker, shares, average_price) VALUES (?, ?, ?)', 
                            (ticker, stock_data['shares'], stock_data['average_price']))
                for transaction in st.session_state['transactions']:
                    c.execute('INSERT INTO transactions (ticker, shares, price, type) VALUES (?, ?, ?, ?)', 
                            (transaction['ticker'], transaction['shares'], transaction['price'], transaction['type']))
                conn.commit()
            ...
            def main():
                ...
                for transaction in st.session_state['transactions']:
                    st.markdown(f"- **{transaction['type'].capitalize()}** {transaction['shares']} shares of {transaction['ticker']} at ${transaction['price']:,.2f} per share.")

                st.markdown("### 📑 Export Report")
                pdf_data = export_to_pdf(portfolio_value)
                st.download_button(label="Export to PDF", data=pdf_data, file_name='portfolio_report.pdf', mime='application/pdf')
